...
def compute_model_agg(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for lang in LANGS:
        ...
        for col in lang_indicator_cols:
            if df_lang[col].nunique() <= 1:
                continue

            X = sm.add_constant(df_lang[col])
            y = df_lang["acc"]
            try:
            ...
            p_pos = sigma(intercept + coef)
            p_neg = sigma(intercept - coef)
            delta = p_pos - p_neg
    ...
    # Per-example interaction: acc ~ is_en + metric + is_en*metric
...
def main() -> None:
    ...
    fig.savefig(f"{DATASET}_agg.png", dpi=500, bbox_inches="tight")
