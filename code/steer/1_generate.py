...
def split_thinking_and_answer(output_text: str):
...
def eval_distill_mgsm(
    ...
    with open(input_path, "r", encoding="utf-8") as f_in, open(output_path, "a", encoding="utf-8") as f_out:
        with tqdm(total=total_to_run, desc=f"{prompt_lang}", unit="ex") as pbar:
            for line in f_in:
                data = json.loads(line)
                ...
                for out in outputs[0].outputs:
                    ...
                    thinking_content, answer_content = split_thinking_and_answer(output_text)
                    candidates.append(
                        {
                ...
                thinking_content = candidates[0]["reasoning_trace"] if candidates else ""
                answer_content = candidates[0]["response"] if candidates else ""

                print("="*50, " 💭 Thinking 💭 ", "="*50)
                print(thinking_content)
                print("="*50, " 🤖 Answer 🤖 ", "="*50)
                print(answer_content)
                print("="*50, " 🔑 Gold Answer 🔑 ", "="*50)
                print(data["answer"])
                ...
                result = {
                    ...
                    "replaced": str(data["replaced"])
                }
                f_out.write(json.dumps(result, ensure_ascii=False) + "\n")
                pbar.update(1)


def eval_distill_aime(
    tokenizer,
    llm,
    input_path,
    output_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--languages", type=str, nargs="+", default=["bn", "de", "en", "es", "fr", "ru", "sw", "te", "th", "zh"])
    ...
    for language in args.languages:
        input_path = f"../data/{args.data_type}/{language}.jsonl"
        output_path = f"res/{args.data_type}/{args.save_name}/{language}.jsonl"
        output_dir = os.path.dirname(output_path)
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)

        if "mgsm" in args.data_type:
            eval_distill_mgsm(
                tokenizer=tokenizer,
                llm=llm,
